...
public class QuestionActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstancesState) {
        ...
        if(subject.equals("mobile_intruduction")){
            max_number = 250;
    ...
    class GetData extends AsyncTask<int[], Integer, String> {
        ...
        protected String doInBackground(final int[]... strings) {
            Log.e(TAG,  "Get subject : " + subject);
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference(subject);
            ref.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    int[] randResult = strings[0];
                    for (int i = 0; i < randResult.length; i++) {
                        Log.e(TAG, dataSnapshot.getChildren() + " : Get Key"  );
                        TestItem items = dataSnapshot.child(randResult[i] + "").getValue(TestItem.class);
                        testLists.add(items);
                    }
                    updateView(page);
                }
